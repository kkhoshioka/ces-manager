...
// The original file is checked out again. It has CRLF.
code = code.replace(/\r\n/g, '\n');

const interfaceBlock = `interface Customer {
    id: number;
    code: string;
    name: string;
    address: string | null;
...
const newInterfaceBlock = `interface CustomerContact {
    id?: number;
    name: string;
...
const formDataBlock = `    const [formData, setFormData] = useState<Omit<Customer, 'id'>>({
        code: '',
        name: '',
        address: '',
        phone: '',
...
const newFormDataBlock = `    const [formData, setFormData] = useState<Omit<Customer, 'id'>>({
        code: '',
        name: '',
        address: '',
        phone: '',
...
const openEditBlock = `    const openEdit = (customer: Customer) => {
...
const newOpenEditBlock = `    const openEdit = (customer: Customer) => {
        setEditingId(customer.id);
        setFormData({
            code: customer.code,
            name: customer.name,
...
const modalRegex = /<div className=\{styles\.modal\} style=\{\{ maxWidth: '600px' \}\}>([\s\S]*?)<form onSubmit=\{handleSubmit\} className=\{styles\.form\}>([\s\S]*?)<\/form>\s*<\/div>\s*<\/div>\s*\)\}\s*<\/div>\s*\);\s*\}\s*;\s*export default CustomerMaster;/;
...
if (!code.includes(interfaceBlock)) console.log("Missing interface");
if (!code.includes(formDataBlock)) console.log("Missing formData");
if (!code.includes(openEditBlock)) console.log("Missing openEdit");
if (!code.includes(openAddBlock)) console.log("Missing openAdd");
...
console.log("Updated CustomerMaster.tsx directly via JS.");
